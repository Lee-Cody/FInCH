...
#		for key, value in cfg.items('fileIdentifiers'):
	...
	def _getColourDeconvolutionTxtFile(self,default_settings):
		...
		if os.path.exists(cdtxt_path) is False:
			...
			cdtxt_missing_example_line = False
			cdtxt_empty = False
			try:
				with open(cdtxt_path,"rb") as cdtxt:
					...
					if len(cdtxt_contents) > 0:
						if cd_txt_internal[0] != cdtxt_contents[0]:
							cdtxt_missing_example_line = True
						for line in cdtxt_contents:
							found_def_spectra = False
							if line == cd_txt_internal[0]:
								continue
							linesplit = str(line).split(',')[0]
						...
						cdtxt_empty = True
					if (cdtxt_empty) or (cdtxt_missing_example_line) or (len(found_spectra) != len(default_spectra)):
						...
						with open(cdtxt_path,'wb') as cdtxt_out:
							if cdtxt_empty:
								...
								for temp_spectra in cd_txt_internal:
									if temp_spectra.split(',')[0] in default_spectra:
										if temp_spectra.split(',')[0] in found_spectra:
											continue
										else:
											cdtxt_contents.insert(1,temp_spectra)
							for line in cdtxt_contents:
								cdtxt_out.write(line)
			except Exception as e:
