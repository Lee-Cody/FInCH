...
class dataManager():
	...
	def _getIcon(self):
		...
		try:
			...
			print"Error dm._getIcon(): ",repr(e)
			implogo = None
			impicon = None
	...
	def checkINI(self,force_new_ini = False):
		...
		try:
			self.cdtxt_spectra,self.default_cd_bool,errorbool = self._getColourDeconvolutionTxtFile(default_settings)
		except Exception as e:
			errorbool = True
			self.menuObj.errorWindow(14,exit=False,error=e)
			if self.data_enabled == 'error':
				return errorbool
			else:
				errorbool = False
		if errorbool:
			self.menuObj.errorWindow(14,exit=False)
			if self.data_enabled == 'error':
				return errorbool
			else:
				errorbool = False
		if force_new_ini:
			...
			return errorbool
		if self.ini_exists:
			IJ.log("Loading settings from FInCH.ini...")
			errorbool = self._getINISettings()
		else:
			IJ.log("FInCH.ini configuration file not found. Starting initial FInCH setup...")
			errorbool = self._generateINI()
		return errorbool
		
	def _getINISettings(self):
		#get configuration settings from FInCH.ini:
		errorbool = False
		ini_settings = {'fileIdentifiers':{}}
		tfs = []
		tfs_matched = []
		spectra = []
...
#		ini_settings['fileIdentifiers'] = {}
		...
		try:
			#attempt to read FInCH.ini:
			cfg.read(ini_path)
		except Exception as e:
			self.menuObj.errorWindow(13,exit=False,error=e)
			if len(cfg_sections_temp) > 0:
				for section in cfg_sections_temp:
					cfg.remove_section(section)
			errorbool = self._generateINI()
			return errorbool
		try:
			#check for old ini settings:
			if cfg.has_section('transcriptionFactors'):
				tf_options = cfg.options('transcriptionFactors')
				tf_items = cfg.items('transcriptionFactors')
				if cfg.has_section('fileIdentifiers') is False:
					cfg.add_section('fileIdentifiers')
				for key,value in tf_items:
					cfg.set('fileIdentifiers',key,value)
				cfg.remove_section('transcriptionFactors')
				with open(self.ini_path, 'w') as configfile:
					cfg.write(configfile)
			#attempt to parse FInCH.ini:
			cfg_tf_items = cfg.items('fileIdentifiers')
			...
			errorbool = self._generateINI()
			return errorbool
		#verify threshold cutoff value from file:
		if (cfg_thresh < 1) or (cfg_thresh > 255):
			e = "thresholdCutoffValue in FInCH.ini is invalid!"
			self.menuObj.errorWindow(3,exit=False,error=e)
			if len(cfg_sections_temp) > 0:
				for section in cfg_sections_temp:
					cfg.remove_section(section)
			errorbool = self._generateINI()
			return errorbool
		#verify file identifiers from file, match them to spectra from colourdeconvolution.txt, and store them as usable variables:
#		for key, value in cfg.items('fileIdentifiers'):
		for key, value in cfg_tf_items:
			found_tf = False
			found_spectra = False
			#tfs and spectra start as empty lists; if the key/value are not found, add them; filters out duplicates as we iterate through file tfs
			for tf_temp in tfs:
				if key == tf_temp:
					found_tf = True
					break
			...
			if found_tf is False:
				...
				ini_settings['fileIdentifiers'][key] = value
			if (found_tf) or ((r"\n" or "=") in (repr(key) or repr(value))):
				#duplicate file identifier or invalid formatting found in FInCH.ini; ask if user wants to redo initial setup
		...
		self.stains = ini_settings['fileIdentifiers']
	...
	def _generateINI(self):
		...
		default_settings = self.default_ini_settings
		ini_settings = {}
		gd = fm.createDialog('ini_setupMenu')
		gd.showDialog()
		if (gd.wasOKed()):
			create_ini = True
		elif gd.wasCanceled():
		...
		if create_ini:
			if defaults:
				...
				for tf_match in user_ini_settings['tfs_matched']:
					cfg.set('fileIdentifiers',tf_match[0],tf_match[1])
					ini_settings['fileIdentifiers'][tf_match[0]] = tf_match[1]
				cfg.add_section('Settings')
	...
	def _parseTFSettings(self,settings):
		...
		tfs = []
		spectra = []
		for key, value in settings['fileIdentifiers'].items():
			if key not in tfs:
				tfs.append(key)
	...
	def _getColourDeconvolutionTxtFile(self,default_settings):
		errorbool = False
		default_spectra = default_settings['defaultSpectra']
		cd_txt_internal = default_settings['colourdeconvolutiontxt']
		cdtxt_path = os.path.join(self.plugins_dir,"colourdeconvolution.txt")
		self.cdtxt_path = cdtxt_path
		cdtxt_lines = []
		default_cd = []
		default_cd_bool = []
		found_spectra = []
		if os.path.exists(cdtxt_path) is False:
			#colourdeconvolution.txt not found; create it from default settings in FInCH_.py
			IJ.log("colourdeconvolution.txt NOT found; creating new file...")
			try:
				with open(cdtxt_path,"wb") as cdtxt:
					for line in cd_txt_internal:
						cdtxt.write(line)
						linesplit = str(line).split(',')[0]
						cdtxt_lines.append(str(linesplit))
						for spectra in default_spectra:
							if spectra.lower() in linesplit.lower():
				...
				print"Error creating new colourdeconvolution.txt file: ",repr(e)
				errorbool = True
				return None,None,errorbool
		else:
			#colourdeconvolution.txt found; read it and make sure it has defaults from FInCH_.py 
#			IJ.log("colourdeconvolution.txt found; verifying...")
			cdtxt_missing_internal_spectra = False
			missing_internal_spectra = []
			cdtxt_empty = False
			try:
				with open(cdtxt_path,"rb") as cdtxt:
					cdtxt_contents = cdtxt.readlines()
					if len(cdtxt_contents) > 0:
						if cd_txt_internal[0] != cdtxt_contents[0]:
							cdtxt_missing_internal_spectra = True
						for i,cdtxt_line in enumerate(cdtxt_contents):
							if len(cdtxt_line) == 0:
									continue
							cdtxt_parts = str(cdtxt_line).partition(",")
							cdtxt_name = cdtxt_parts[0]
							for int_spectra in cd_txt_internal:
								int_spectra_temp = int_spectra.rstrip('\n')
								if "\n" not in cdtxt_line:
									cdtxt_line = u"{}\n".format(cdtxt_line)
									cdtxt_contents[i] = cdtxt_line
								int_spectra_parts = str(int_spectra).partition(',')
								if (str(cdtxt_line) == str(int_spectra)) or (str(cdtxt_line) == str(int_spectra_temp)):
									found_spectra.append(int_spectra)
								elif cdtxt_name == int_spectra_parts[0]:
									if len(cdtxt_line.split(",")) == len(int_spectra.split(",")):
										cdtxt_line_details = cdtxt_parts[1] + cdtxt_parts[2]
										cdtxt_line_tempname = cdtxt_name + "_u"
										cdtxt_line_new = cdtxt_line_tempname + cdtxt_line_details
										cdtxt_contents[i] = int_spectra
										missing_internal_spectra.append(cdtxt_line_new)
										found_spectra.append(int_spectra)
										continue
						for int_spectra in cd_txt_internal:
							found_internal_spectra = False
							for found in found_spectra:
								if int_spectra == found:
									found_internal_spectra = True
									continue
							if found_internal_spectra is False:
								missing_internal_spectra.append(int_spectra)
					else:
						cdtxt_empty = True
					if (cdtxt_empty) or (cdtxt_missing_internal_spectra) or (len(missing_internal_spectra) > 0):
						#Didn't find all of FInCH's default cd spectra in colourdeconvolution.txt; add them:
#						IJ.log("colourdeconvolution.txt format was missing necessary information. Adding to file...")
						cdtxt_lines = []
						with open(cdtxt_path,'wb') as cdtxt_out:
							if cdtxt_empty:
								cdtxt_contents = cd_txt_internal
							else:
								if cd_txt_internal[0].partition('\n')[0] != cdtxt_contents[0].partition('\n')[0]:
									if cdtxt_contents[0].startswith("#"):
										cdtxt_contents[0] = cd_txt_internal[0]
									else:
										cdtxt_contents.insert(0,cd_txt_internal[0])
								if len(missing_internal_spectra) > 0:
									for spectra in missing_internal_spectra:
										cdtxt_contents.append(spectra)
							for line in cdtxt_contents:
								cdtxt_out.write(str(line))
			except Exception as e:
				print"Error verifying colourdeconvolution.txt: ",repr(e)
				errorbool = True
				return None,None,errorbool
			for line in cdtxt_contents:
				if str(line).startswith('#'):
					continue
				found_def_spectra = False
				linesplit = str(line).partition(',')[0]
				cdtxt_lines.append(linesplit)
				if default_spectra is not None:
					for spectra in default_spectra:
						if spectra.lower() in str(linesplit).lower():
							found_def_spectra = True
							found_spectra.append(spectra)
				default_cd_bool.append(found_def_spectra)
		return cdtxt_lines, default_cd_bool, errorbool
	...
	def gatherImages(self):
		...
		for foundfile in os.listdir(self.folderpath):
			...
			if found_identifier is False:
				...
				IJ.log("invalid filename found: ".format(fname))
				IJ.log("Please check documentation for required file naming conventions!")
				return errorbool
			if self.upperlower:
				if "lower" in fname:
	...
	def getData(self,fimp):
		...
		for cell in cells:
			if (cell.data == None):
				self.menuObj.errorWindow(18,error=[fimp.title,cell.name,cell.data,cell.offimage])
				errorbool = True
				return errorbool
...
###########################################################################################		
	def processImages(self):
		...
		getDataBool = self.getDataBool
		threshlevel = self.ini_settings['thresholdCutoffValue']
...
#				imp = fimp.imp
				...
				if os.path.exists(savepath):
					...
					try:
						IJ.run(imp, "Colour Deconvolution", "vectors=[{}] hide".format(fimp.stain))
					except Exception as e:
						IJ.log("Could not load or process file: {}".format(fimp.title))
						...
						fm.errorWindow(0)
						return errorbool
					IJ.log("Performing color deconvolution on [{}], spectra used: [{}]".format(fimp.title,fimp.stain))
					for title in WindowManager.getImageTitles():
				...
				if not os.path.exists(threshSavePath):
					try:
						...
						IJ.log("Could not threshold image or could not load/process file:".format(fimp.title))
		...
		for fimp in self.fimplist:
			...
			if title in opentitles:
				...
				try:
					...
					print"Error in DataManager.dataManager.showImages(): ",repr(e)
					errorbool = True
					fm.errorWindow(6)
					return errorbool
				...
				if WindowManager.getImage(thresh_title) is not None:
					...
					if os.path.exists(fimp.thresh_savepath):
						try:
							...
							fm.errorWindow(17,exit=False,error=[fimp.title,e])
							imp_thresh = None
			...
			if (imp_thresh is None) and (os.path.exists(threshSaveDir)):
				...
				if os.path.exists(imp_thresh_savepath):
					try:
						imp_thresh = IJ.openImage(imp_thresh_savepath)
						imp_thresh.show()
						if (fimp.thresh_image is None) or (fimp.thresh_image != imp_thresh):
							fimp.thresh_image = imp_thresh
