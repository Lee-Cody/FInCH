	...
	def findStain(self,stain_options,impicon=None):
		...
		if stain is None or (stain == ""):
			if (len(colors) == 0):
				...
				alertgd.addMessage("If you have changed the stains variable near the top of FInCH_.py, please double check that the dictionary keys and values are correctly entered")
				alertgd.showDialog()
				errorbool = True
				return errorbool
