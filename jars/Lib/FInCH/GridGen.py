...
class fImp:
	instances = []
	def __init__(self,imp,uline=None,grid=None):
		self.imp = imp
		...
		self.name = getNameFromTitle(self.title)
		self.userline = uline
	...
	def findStain(self,impicon=None):
		errorbool = False
		ftitle = self.title.lower()
		if ftitle.startswith("bcat"):
			stain = "dab"
		elif ftitle.startswith("cam"):
			stain = "dab"
		elif ftitle.startswith("tgfb"):
			stain = "dab"
		elif ftitle.startswith("fgf"):
			stain = "dab"
		elif ftitle.startswith("bmp"):
			stain = "vr"
		elif ftitle.startswith("dkk"):
			stain = "vr"
		elif ftitle.startswith("ihh"):
			stain = "vr"
		elif ftitle.startswith("wnt"):
			stain = "vr"
		else:
			print "couldn't figure out the stain for: ", self.title
			gd = GenericDialogPlus("Stain")
			...
			stains = ["DAB", "Vector Red"]
			gd.addChoice("Select Stain", stains, stains[0])
			gd.showDialog()
			if gd.wasCanceled():
				errorbool = True
				return errorbool
		...
		if stain == "":
				...
				alertgd.addMessage("Function: getStain()")
				alertgd.addMessage("Unable to determine stain for file: {}".format(self.title))
				alertgd.showDialog()
				errorbool = True
				return errorbool
		self.stain = stain
